greedy = function(model, data = NULL, center = FALSE, inform = "AIC", digits = 5){
  ...
  do.this.stuff = function(){
    ...
    #problem right now is it is trying to drop a squared term even though it exists in a interaction
    ...
    squared.terms.pos = grep("^", testsat1, fixed = T)
    squared.terms.names = testsat1[squared.terms.pos]
    squared.terms.pos = grep(":", squared.terms.names, fixed = T, invert = T)
    squared.terms.names = squared.terms.names[squared.terms.pos]
  
    # print("---all.test.pos is---")
    # print(all.test.pos)
    # print("--------")
    ...
    # print("---squared.terms.names---")
    # print(squared.terms.names)
    # print("--------")
    
    test.list = list()
    
    #this block removes the main effects from tempsat
    #ideally, it will also remove the squared terms that appear in interactions
    if(!is.null(testsat)){
      for(x in 1:length(testsat)){
        #this section will change the cat main effects in testsat to their form without levels
        ...
        # print("---testsat1[test first]---")
        # print(testsat1[test])
        
        test = test + length(testsat)
        
        # print("---test second is---")
        # print(test)
        
        # print("---rownames(sumsat)[test second]---")
        # print(rownames(sumsat)[test])
...
test.models = function(){
  ...
  # model = lm(Y ~ (V01 + V02 + V03 + I(V01^2) + I(V02^2) + I(V03^2))^2, data = data)
  model = lm(Y ~ (X1C + X2C + X3C + X4C + X5C)^2, data = data)
  print(model)
  ...
  model = lm(area ~ (X + Y + month + day + FFMC + DMC + DC + ISI + temp + RH + wind + rain + I(FFMC^2) + I(DMC^2) + I(DC^2) + I(ISI^2) + I(temp^2) + I(RH^2) + I(wind^2) + I(rain^2)), data = data)
