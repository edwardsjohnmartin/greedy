...
# You can use ctrl + shft + C to uncomment and recomment blocks of code
# I used this just so I can test this with each dataset quickly
...
#grdy <- greedy(log(Y) ~ log(Xb) + Xspec + Xg + Xl, data = case0902)

# readFile = "Datasets/Case1202.csv"
# case1202 <- read.csv(file = readFile, header = FALSE, skip = 1)
# case1202$V2 = NULL
# case1202$V3 = NULL
# names(case1202) <- c("Y", "V1", "V2", "V3", "V4")
# grdy <- greedy(log(Y) ~ log(V1) + V2 + V3 + V4, data = case1202)
# 
...
# fbData$V6 = NULL
# fbData$V7 = NULL
# names(fbData) <- c("Y", "V1", "V2", "V3", "V4", "V5", "V6", "V7", "V8", "V9", "V10", "V11", "V12")
# grdy <- greedy(log(Y) ~ log(V1) + V2 + V3 + V4 + V5 + V6 + V7 + V8 + V9 + V10 + V11 + V12, data = fbData)

# readFile = "Datasets/concreteData.csv"
# concreteData <- read.csv(file = readFile, header = FALSE, skip = 1)
# names(concreteData) <- c("Y", "V1", "V2", "V3", "V4", "V5", "V6", "V7", "V8")
# grdy <- greedy(log(Y) ~ V1 + V2 + V3 + V4 + V5 + V6 + V7 + V8, data = concreteData)

# readFile = "Datasets/whiteWineData.csv"
# wineData <- read.csv(file = readFile, header = FALSE, skip = 1)
# names(wineData) <- c("Y", "V1", "V2", "V3", "V4", "V5", "V6", "V7", "V8", "V9", "V10", "V11")
# grdy <- greedy(log(Y) ~ V1 + V2 + V3 + V4 + V5 + V6 + V7 + V8 + V9 + V10 + V11, data = wineData)

# readFile = "Datasets/powerPlantData.csv"
# powerPlantData <- read.csv(file = readFile, header = FALSE, skip = 1)
# names(powerPlantData) <- c("V1", "V2", "V3", "V4", "Y")
# grdy <- greedy(log(Y) ~ V1 + V2 + V3 + V4, data = powerPlantData)

# readFile = "Datasets/parkinsonsData.csv"
# parkinsonData <- read.csv(file = readFile, header = FALSE, skip = 1)
# parkinsonData$V1 = NULL
# parkinsonData$V2 = NULL
# parkinsonData$V3 = NULL
# parkinsonData$V4 = NULL
# # This dataset has 2 response variables, uncomment the next line and comment the line after to change the response variable
# parkinsonDataV5 = NULL
# # parkinsonData$V6 = NULL
# names(parkinsonData) <- c( "Y", "V1", "V2", "V3", "V4", "V5", "V6", "V7", "V8", "V9", "V10", "V11", "V12", "V13", "V14", "V15", "V16")
# grdy <- greedy(log(Y) ~ V1 + V2 + V3 + V4 + V5 + V6 + V7 + V8 + V9 + V10 + V11 + V12 + V13 + V14 + V15 + V16, data = parkinsonData)
# 
# library("profvis")
# g = profvis(greedy(log(Y) ~ V1 + V2 + V3 + V4 + V5 + V6 + V7 + V8 + V9 + V10 + V11 + V12 + V13 + V14 + V15 + V16, data = parkinsonData))
# print(g)
