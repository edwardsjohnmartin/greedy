greedy = function(model, data = NULL, center = FALSE, inform = "AIC", digits = 5){
  ...
  do.this.stuff = function(){
    ...
    test = grep(":", rownames(sumsat), fixed = T, invert = T)
    
    main.effects.names = grep("^", rownames(sumsat)[test], fixed = T, invert = T, value = T)
    squared.terms.names = grep("^", rownames(sumsat)[test], fixed = T, value = T)
    interactions.names = grep(":", rownames(sumsat), fixed = T, value = T)
    
    remove.list = list()
    
    if(length(main.effects.names) > 0){
      for(i in 1:length(main.effects.names)){
        name.row = match(main.effects.names[i], namesMat)
        new.term = rownames(namesMat)[row(namesMat)[name.row]]
        test7 = grep(new.term, main.effects.names, value = T)

        if(length(squared.terms.names) > 0){
          test3 = grep(main.effects.names[i], squared.terms.names)
          if(length(test3) > 0){
            remove.list = append(remove.list, main.effects.names[i])
          }
        }
        
        if(length(interactions.names) > 0){
          test1 = grep(main.effects.names[i], interactions.names)
          if(length(test1) > 0){
            if(length(test7) > 0){
              remove.list = append(remove.list, test7)
            } else {
              remove.list = append(remove.list, main.effects.names[i])
            }
          }
    ...
    if(length(squared.terms.names) > 0){
      for(i in 1:length(squared.terms.names)){
        if(length(interactions.names) > 0){
          test2 = grep(squared.terms.names[i], interactions.names, fixed = T, value = T)
          if(length(test2) > 0){
            remove.list = append(remove.list, squared.terms.names[i])
          }
        }
      }
    }
    
    remove.list = unlist(unique(remove.list))
    
    test4 = match(remove.list, rownames(sumsat))
    
    if(length(test4) > 0){
      tempsat = sumsat[-test4,]
    } else {
      tempsat = sumsat
    }
    ...
    if(class(tempsat)=="numeric"){
      ...
      rownames(tempsat) = rownames(sumsat)[-test4]
    }
    ...
    if(length(attr(terms(model), "term.labels")) > 1){
      ...
      if(class(model)[1] == "lm"){
        ...
        model = glm(terms.new, family = fam, data = data)
        f = formula(model)
      }
    } else {
      if(class(model)[1] == "lm"){
        ...
        new.form = as.formula(paste(dep.name, "~", 1, sep = ""))
        model = glm(new.form, family = fam, data = data)
  ...
  if(class(model)[1] == "glm"){
    fam = as.character(model$family[1])
  }
  ...
  if(class(model)[1] == "lm"){
    ...
    best <- glm(noquote(best), family = fam, data = data)
  }
...
test.models = function(){
  ...
  # crabs dataset = family poisson
  # beetle dataset = family binomial
  ...
  data = read.csv(file = "Datasets/whiteWineData.csv", col.names = wine.names)
  model = lm(qual ~ (vol.acid + cit.acid + res.sugar + chlor + free.sulfur + total.sulfur + dens + PH + sulph + alcohol + fixed.acid + I(vol.acid^2) + I(cit.acid^2) + I(res.sugar^2) + I(chlor^2) + I(free.sulfur^2) + I(total.sulfur^2) + I(dens^2) + I(PH^2) + I(sulph^2) + I(alcohol^2) + I(fixed.acid^2))^2, data = data)
  ...
  # grades.colClasses = c("sex"="factor","address"="factor", "Pstatus"="factor", "schoolsup" = "factor")
  ...
  # print(greedy.test$best$family[1])
  # print("------------greedy AIC----------------")
  # print(AIC(greedy.test$best))
  # print("------------greedy Total Time---------")
  # print(system.time(greedy(model, data = data)))
  
  stepaic.test = stepAIC(model, data = data, trace = T)
  
  # print(system.time(stepaic.test <- stepAIC(model, data = data, trace = F)))
  
  # print(AIC(stepaic.test))
    # stepaic.test = stepAIC(model, data = data, trace = F)
  
  # print(system.time(stepaic.test))
