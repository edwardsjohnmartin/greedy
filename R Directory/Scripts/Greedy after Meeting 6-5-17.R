...
XC1 = rep(c("a", "b", "c"), each = 6)
XC2 = rep(c("a", "b", "c"), length = 18)
XQ1 = runif(18, 1, 10)
Y = rnorm(18)

model = lm(Y ~ XC1 + XC2 + XC1:XC2)

#formula changed to lm so the user can generate own model
  ...
  if(steps==1) tab[1,] = c(deparse(formula(test$terms)), " ", round(inf(test, inform), digits = digits)) # intercept only model
    ...
    drops <-function(mod1, X, data){
      ...
      if(np > 2){
        ...
        # print("-----------")
        # print("This is mod.terms at the beginning of the function")
        # print(mod.terms)
        print("-----------")
        print("This is sumsat at the beginning of the function")
        print(sumsat)
        print("-----------")
        
        drop1 = getTermToDrop(sumsat, mod.terms)
        print(paste("BEFORE: drop1 before the categ part changes it", drop1))
        
        if(Xcn >= 1){
          if(drop1 == "V3:CATPhoto"){
            print("-----------")
            print("This is Xlm at the beginning of the function")
            print(Xlm)
            print("-----------")
          }
          drop1 <- Xlm[,1][Xlm[,2]==drop1]
          print(paste("AFTER: drop1 after the categ part changes it", drop1))
...
# Original lm is creating a bad lm
# all residuals are 0, no residual degrees of freedom
# 294 coefficients not defined because of singularities

# readFile = "Datasets/Case0902.csv"
# data <- read.csv(file = readFile, header = FALSE, skip = 1)
# digits = 5
# inform = "AIC"
# names(data) <- c("Xspec", "Y", "Xb", "Xg", "Xl")
# formula = log(Y) ~ log(Xb) + Xg + Xl + Xspec

# Original lm
# Residuals:
# Min       1Q       Median   3Q       Max 
# -0.38464 -0.03980  0.01566  0.05025  0.25373
# 21 coefficients not defined because of singularities
# Residual standard error: 0.09249 on 387 degrees of freedom
# (4 observations deleted due to missingness)
# Multiple R-squared:  0.6764,	Adjusted R-squared:  0.586 
# F-statistic: 7.489 on 108 and 387 DF,  p-value: < 2.2e-16
...
data$V6 = NULL
data$V7 = NULL
names(data) <- c("Y", "CAT", "V2", "V3", "V4", "V5", "V6", "V7", "V8", "V9", "V10", "V11", "V12", "V13")
formula = log(Y) ~ CAT + V2 + V3 + V4 + V5 + V6 + V7 + V8 + V9 + V10 + V11 + V12 + V13

g = greedy(formula, data = data)